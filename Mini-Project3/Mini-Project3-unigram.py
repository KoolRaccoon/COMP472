...
output_files = ["n_gram_models/unigramEN.txt", "n_gram_models/unigramFR.txt","n_gram_models/unigramRO.txt"]
...
for i in range (0, len(languages)):
    incrementor = 0
    for letter in Unigram_list:
        letter_frequencies[i].append(train_corpuses[i].count(letter))
        n_gram_probs[i].append((letter_frequencies[i][incrementor]+1) / (len(train_corpuses[i]))+(1*len(Unigram_list)))
...
def calculate_sentence_probability(Languages = None, sentence = None, n_gram_probs = None, n_gram_list=None, train_corpus = None, smoothing=None):
    ...
    for letter in sentence:
        uni_string = "UNIGRAM: "+ letter
        string_outputs.append(uni_string)
        # for language in languages:
        for i in range(0, len(languages)):
            letter_prob = get_ngram_probability(character = letter, n_gram_probs = n_gram_probs[i], n_gram_list=n_gram_list, train_corpus_len = len(train_corpus[i]), smoothing=smoothing)
            sentence_log_prob[i] = sentence_log_prob[i] + math.log10(letter_prob)
            string_outputs.append(print_log_prob(Languages[i], sentence_log_prob[i], letter_prob, letter))
